...
const CurriculumManagement = () => {
  ...
  const [unitForm, setUnitForm] = useState({ title: '', category: '', description: '', unit_number: 1, icon: 'biotech', color_theme: 'primary' });

  const [showLessonModal, setShowLessonModal] = useState(false);
  const [selectedUnitId, setSelectedUnitId] = useState(null);
  const [editingLesson, setEditingLesson] = useState(null);
  const [lessonForm, setLessonForm] = useState({ title: '', description: '', lesson_number: 1 });

  const [showVideoModal, setShowVideoModal] = useState(false);
  const [selectedLessonId, setSelectedLessonId] = useState(null);
  const [editingVideo, setEditingVideo] = useState(null);
  const [videoForm, setVideoForm] = useState({ title: '', duration: '30:00', video_url: '', pdf_url: '', video_order: 1 });
  ...
  return (
    <div className="flex flex-col gap-8 p-6">
      ...
      <div className="flex flex-col md:flex-row-reverse justify-between items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h2 className="text-3xl font-bold text-[#006d35]">إدارة المنهج والدروس</h2>
          <p className="text-lg text-gray-600">إضافة وتعديل الوحدات والدروس ومقاطع الفيديو حسب المرحلة الدراسية</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              setEditingUnit(null);
              setUnitForm({ title: '', category: '', description: '', unit_number: units.length + 1, icon: 'biotech', color_theme: 'primary' });
      ...
      {/* Content Grid */}
      {loading ? (
        <div className="text-center py-16 text-2xl text-[#006d35] font-bold">جاري تحميل بيانات المنهج...</div>
      ) : units.length === 0 ? (
        <div className="bg-white p-12 text-center text-xl text-gray-500 rounded-2xl border">
          لا توجد وحدات دراسية حالياً لهذه المرحلة. اضغط على "إضافة وحدة جديدة" للبدء.
        </div>
      ) : (
        <div className="flex flex-col gap-6">
          {units.map((unit) => (
            <div key={unit.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex flex-col gap-6">
              {/* Unit Header */}
              <div className="flex justify-between items-center bg-[#e1fae7] p-4 rounded-xl border border-[#00e475]/30">
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-3xl text-[#006d35]">{unit.icon || 'biotech'}</span>
                  <div>
                    <h3 className="text-2xl font-bold text-[#0a2014]">الوحدة {unit.unit_number}: {unit.title}</h3>
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      ...
                      setLessonForm({ title: '', description: '', lesson_number: (unit.lessons?.length || 0) + 1 });
              ...
              <div className="flex flex-col gap-4 pr-4 border-r-4 border-[#006d35]/20">
                {unit.lessons?.map((lesson) => (
                  <div key={lesson.id} className="bg-[#f8fafc] p-4 rounded-xl border flex flex-col gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            ...
                            setVideoForm({ title: '', duration: '30:00', video_url: '', pdf_url: '', video_order: (lesson.videos?.length || 0) + 1 });
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                      {lesson.videos?.map((vid) => (
                        <div key={vid.id} className="bg-white p-3 rounded-lg border flex justify-between items-center shadow-xs">
                          <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-[#006d35]">play_circle</span>
                            <div>
                              <div className="font-bold text-gray-800">{vid.title}</div>
                              <div className="text-xs text-gray-500">مدة الفيديو: {vid.duration}</div>
                            </div>
                          </div>
      ...
      {showUnitModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-lg flex flex-col gap-4">
            <h3 className="text-2xl font-bold text-[#006d35]">{editingUnit ? 'تعديل الوحدة' : 'إضافة وحدة جديدة'}</h3>
            <form onSubmit={handleSaveUnit} className="flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                <button type="submit" className="bg-[#006d35] text-white px-6 py-3 rounded-xl font-bold text-lg flex-1">
                  حفظ
                </button>
                <button type="button" onClick={() => setShowUnitModal(false)} className="bg-gray-200 text-gray-800 px-6 py-3 rounded-xl font-bold text-lg">
                  إلغاء
                </button>
              </div>
            </form>
      ...
      {showLessonModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-lg flex flex-col gap-4">
            <h3 className="text-2xl font-bold text-[#006d35]">{editingLesson ? 'تعديل الدرس' : 'إضافة درس جديد'}</h3>
            <form onSubmit={handleSaveLesson} className="flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                ...
                <button type="button" onClick={() => setShowLessonModal(false)} className="bg-gray-200 text-gray-800 px-6 py-3 rounded-xl font-bold text-lg">
                  إلغاء
                </button>
              </div>
            </form>
      ...
      {showVideoModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-lg flex flex-col gap-4">
            <h3 className="text-2xl font-bold text-[#006d35]">{editingVideo ? 'تعديل الفيديو' : 'إضافة فيديو جديد'}</h3>
            <form onSubmit={handleSaveVideo} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="font-bold text-gray-700">رابط الفيديو أو رفعه من الجهاز:</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-2">
                <label className="font-bold text-gray-700">ملف ملخص PDF (اختياري):</label>
                <input
                  type="text"
                  placeholder="مسار PDF"
                  value={videoForm.pdf_url}
                  onChange={(e) => setVideoForm({ ...videoForm, pdf_url: e.target.value })}
                  className="p-3 border rounded-xl text-lg"
                />
                <input
                  type="file"
              ...
              {uploadingFile && <div className="text-blue-600 font-bold">جاري رفع الملف...</div>}

              <div className="flex gap-4 mt-2">
                <button type="submit" disabled={uploadingFile} className="bg-[#006d35] text-white px-6 py-3 rounded-xl font-bold text-lg flex-1">
                  حفظ الفيديو
                </button>
                <button type="button" onClick={() => setShowVideoModal(false)} className="bg-gray-200 text-gray-800 px-6 py-3 rounded-xl font-bold text-lg">
                  إلغاء
                </button>
              </div>
            </form>
