...
const LecturePlayerPage = () => {
  ...
  const fetchVideoDetails = async (id) => {
    ...
    try {
      ...
      const endpoint = id ? `/api/curriculum/video/${id}${studentIdParam}` : `/api/curriculum/video/1${studentIdParam}`;
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[80vh] text-2xl text-[#006d35] font-semibold">
        <span className="material-symbols-outlined animate-spin text-5xl ml-3">sync</span>
        جاري تحميل المحاضرة...
      </div>
    );
  }
  ...
  const currentVideo = videoData?.currentVideo || {
    ...
    pdf_url: '/uploads/sample_biology_summary.pdf',
  };
  ...
  const overallProgress = videoData?.overallProgress || 33;
  const completedCount = videoData?.completedVideosCount || 4;
  const totalCount = videoData?.totalVideosCount || 12;
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 order-2 lg:order-1">
          ...
          <div className="glass-card rounded-2xl p-6">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xl font-bold text-[#006d35]">{overallProgress}% مكتمل</span>
            </div>
            <p className="text-lg text-[#3b4a3d] mb-3">{completedCount}/{totalCount} دروس</p>
          ...
          <div className="glass-card rounded-2xl p-6 flex flex-col gap-4">
            <div className="flex items-center gap-2 pb-2 border-b border-[#006d35]/10">
              <span className="material-symbols-outlined text-[#006d35] text-2xl">format_list_bulleted</span>
              <h3 className="text-2xl font-bold text-[#0a2014]">قائمة الدروس</h3>
            </div>

            <div className="flex flex-col gap-3">
              {playlist.flatMap((lesson) => lesson.videos || []).map((vid, idx) => {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className={`text-xl font-bold ${isPlaying ? 'text-[#006d35]' : 'text-[#0a2014]'}`}>
                          {vid.title}
                        </span>
                        <span className="text-sm text-[#3b4a3d]">
                          {isPlaying ? '45:00 - قيد التشغيل' : `${vid.duration || '30:00'}`}
                        </span>
                      </div>
                    </div>
                  </button>
          ...
          <div className="glass-card rounded-3xl p-8 flex flex-col gap-6">
            ...
            {activeTab === 'overview' && (
              <div className="flex flex-col gap-6 text-[#3b4a3d] text-xl leading-relaxed">
                <p>
                  في هذه المحاضرة، سنستكشف التركيب الجزيئي الدقيق للحمض النووي (DNA). سنبدأ بمراجعة مكونات النيوكليوتيدات وكيفية ترابطها لتكوين الهيكل الحلزوني المزدوج.
                </p>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  >
                    ...
                    <div className="flex flex-col text-right">
                      ...
                      <span className="text-sm font-normal text-[#3b4a3d]">PDF • 2 MB</span>
                    </div>
                  </a>
                  ...
                  >
                    ...
                    <div className="flex flex-col text-right">
                      ...
                      <span className="text-sm font-normal text-[#3b4a3d]">منتدى • 15 تعليق</span>
                    </div>
                  </button>
                </div>
              </div>
            ...
            {activeTab === 'resources' && (
              <div className="flex flex-col gap-4">
                <h4 className="text-2xl font-bold text-[#0a2014]">ملفات الشرح والتلخيص</h4>
                <a
                  href={currentVideo.pdf_url || '#'}
                  download
                ...
                >
                  <div className="flex items-center gap-4">
                    <span className="material-symbols-outlined text-[#b02f00] text-4xl">picture_as_pdf</span>
                    <div>
                      <h5 className="text-2xl font-bold text-[#0a2014]">ملخص وتلخيص سبورة الدرس (PDF)</h5>
                      <span className="text-lg text-[#3b4a3d]">شامل جميع الملاحظات والرسومات التوضيحية</span>
                    </div>
                  </div>
                  <span className="material-symbols-outlined text-3xl text-[#006d35]">download</span>
                </a>
              </div>
            )}
            ...
            {activeTab === 'qa' && (
              <div className="flex flex-col gap-6">
                <h4 className="text-2xl font-bold text-[#0a2014]">أسئلة الطلاب واستفساراتهم</h4>
                <div className="flex flex-col gap-4">
                  <textarea
                  ...
                  ></textarea>
                  <button className="self-end px-8 py-3 rounded-full bg-[#006d35] text-white font-bold text-xl hover:bg-[#005226] transition-all">
                    إرسال السؤال
                  </button>
                </div>
              </div>
