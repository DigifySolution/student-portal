...
const SecureVideoPlayer = ({
  ...
  useEffect(() => {
    ...
    const loadVideo = async () => {
      setLoading(true);
      try {
        const streamEndpoint = videoId ? `/api/curriculum/stream/${videoId}` : videoUrl;

        // If external sample HTTP video, fallback directly or fetch blob
        if (streamEndpoint.startsWith('http://') || streamEndpoint.startsWith('https://')) {
    ...
    >
      ...
      {loading && (
        <div className="absolute inset-0 z-40 bg-black/80 flex flex-col justify-center items-center text-white gap-3">
          <span className="material-symbols-outlined text-5xl animate-spin text-[#00e676]">sync</span>
      ...
      >
        {!isPlaying && !loading && (
          <button
            ...
            className="w-24 h-24 rounded-full bg-[#00e676] text-[#006d35] flex items-center justify-center shadow-2xl hover:scale-110 transition-transform"
          >
      ...
      <div
        ...
        className="absolute bottom-0 inset-x-0 z-30 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4 flex flex-col gap-2 transition-opacity duration-300 opacity-90 hover:opacity-100"
      >
        {/* Timeline Slider */}
        <input
          type="range"
        ...
        <div className="flex justify-between items-center text-white px-2">
          <div className="flex items-center gap-4">
            <button onClick={togglePlay} className="hover:text-[#00e676] transition-colors">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleMute} className="hover:text-[#00e676] transition-colors">
          ...
          <div className="flex items-center gap-4">
              ...
              {[1, 1.25, 1.5, 2].map((speed) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded font-bold transition-colors ${
                    playbackSpeed === speed ? 'bg-[#00e676] text-[#006d35]' : 'hover:bg-white/20'
                  }`}
                >
                  {speed}x
                </button>
            ...
            <button onClick={toggleFullscreen} className="hover:text-[#00e676] transition-colors">
              <span className="material-symbols-outlined text-2xl">
